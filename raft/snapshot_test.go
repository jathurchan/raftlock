...
func TestRaftSnapshot_SnapshotManager_Stop_Scenarios(t *testing.T) {

	t.Run("Stop_NoActiveOperations_ReturnsImmediately", func(t *testing.T) {
		sm, _, _, _, _ := setupSnapshotManager(t)
		...
		if stopDuration > maxExpectedDuration {
			t.Errorf("Stop() took %v with no active operations; expected return < %v", stopDuration, maxExpectedDuration)
		}

		if counter := sm.snapshotOpsCounter.Load(); counter != 0 {
			t.Errorf("Expected snapshotOpsCounter to be 0, got %d", counter)
		}
	})

	t.Run("Stop_GracefulShutdown_WaitsForActiveOperations", func(t *testing.T) {
		sm, _, _, _, _ := setupSnapshotManager(t)
		opSimulatedWorkDuration := 100 * time.Millisecond

		if opSimulatedWorkDuration >= defaultSnapshotStopTimeout {
			t.Fatalf("Test setup error: opSimulatedWorkDuration (%v) must be less than defaultSnapshotStopTimeout (%v)",
				opSimulatedWorkDuration, defaultSnapshotStopTimeout)
		}

		sm.snapshotOpsCounter.Add(1)
		sm.snapshotOpsWg.Add(1)

		operationActuallyDoneSignal := make(chan struct{})
		go func() {
			defer close(operationActuallyDoneSignal)
			time.Sleep(opSimulatedWorkDuration)
			sm.snapshotOpsCounter.Add(-1)
			sm.snapshotOpsWg.Done()
		}()

		startTime := time.Now()
		sm.Stop()
		stopCallDuration := time.Since(startTime)

		select {
		case <-operationActuallyDoneSignal:
		case <-time.After(opSimulatedWorkDuration + 100*time.Millisecond):
			t.Fatal("Simulated operation did not signal completion as expected after Stop() returned.")
		}

		minExpectedWaitDuration := opSimulatedWorkDuration - (30 * time.Millisecond)
		if minExpectedWaitDuration < 0 {
			minExpectedWaitDuration = 0
		}
		if stopCallDuration < minExpectedWaitDuration {
			t.Errorf("Stop() returned in %v, which is less than the effective operation duration of ~%v. Expected it to wait.", stopCallDuration, opSimulatedWorkDuration)
		}

		reasonableMaxDuration := opSimulatedWorkDuration + (500 * time.Millisecond)
		if stopCallDuration >= defaultSnapshotStopTimeout {
			t.Errorf("Stop() took %v, suggesting it hit the full timeout %v, but the operation was short (%v) and should have completed gracefully.", stopCallDuration, defaultSnapshotStopTimeout, opSimulatedWorkDuration)
		} else if stopCallDuration > reasonableMaxDuration {
			t.Logf("Warning: Stop() took %v, which is longer than the expected %v for an operation of %v. This might be due to system load or test flakiness.", stopCallDuration, reasonableMaxDuration, opSimulatedWorkDuration)
		}

		if counter := sm.snapshotOpsCounter.Load(); counter != 0 {
			t.Errorf("Expected snapshotOpsCounter to be 0 after graceful shutdown, got %d", counter)
		}
	})
}
