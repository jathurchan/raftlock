...
func createTestElectionManager(t *testing.T) (*electionManager, *mockStateManager, *mockLogManager, *mockNetworkManager, *mockMetrics, *mockClock, *mockRand) {
...
func TestNewElectionManager(t *testing.T) {
	...
	t.Run("InvalidDependencies", func(t *testing.T) {
		...
		testutil.AssertContains(t, err.Error(), "ID must be provided", "Error message should mention ID")
	})
}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantErr && err != nil {
				testutil.AssertContains(t, err.Error(), tc.errMsg, "Error message mismatch") // Called as a separate statement
			}
		})
	}
}
...
func TestInitialize(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		...
		testutil.AssertPositive(t, uint64(em.randomizedPeriod), "Randomized period should be positive after initialization")
	})
	...
	t.Run("NoNetworkManager", func(t *testing.T) {
		...
		testutil.AssertContains(t, err.Error(), "network manager must be set", "Error message should be descriptive")
	})
}
...
func TestResetTimerOnHeartbeat(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		...
		testutil.AssertNotEqual(t, initialRandomizedPeriod, em.randomizedPeriod, "Expected randomized period to be reset (different value)")
		testutil.AssertPositive(t, uint64(em.randomizedPeriod), "Randomized period should be positive after reset")
	})
	...
	t.Run("ShuttingDown", func(t *testing.T) {
		...
		em.ResetTimerOnHeartbeat()
		testutil.AssertEqual(t, 10, em.electionElapsed, "electionElapsed should not change when shutting down")
	})
}
...
func TestTick(t *testing.T) {
	t.Run("FollowerTick_NoElection", func(t *testing.T) {
		...
		testutil.AssertEqual(t, em.randomizedPeriod, em.electionElapsed, "Election elapsed should increment by 1")
	...
	t.Run("FollowerTick_TriggersElection", func(t *testing.T) {
		...
		testutil.AssertEqual(t, types.RoleCandidate, stateMgr.currentRole, "Role should change to Candidate after election trigger")
		testutil.AssertEqual(t, 0, em.electionElapsed, "Election elapsed should reset after election trigger")
	})
	...
	t.Run("LeaderTick", func(t *testing.T) {
		...
		testutil.AssertEqual(t, initialElapsed, em.electionElapsed, "Leader should not increment election elapsed")
	})
	...
	t.Run("ShuttingDown", func(t *testing.T) {
		...
		ctx := context.Background()
		em.Tick(ctx)
		testutil.AssertEqual(t, initialElapsed, em.electionElapsed, "Tick should do nothing when shutting down")
	})
	...
	t.Run("ContextCancelled", func(t *testing.T) {
		...
		em.Tick(ctx)
		testutil.AssertEqual(t, initialElapsed, em.electionElapsed, "Tick should do nothing when context is cancelled")
	})
	...
	t.Run("StopChannelClosed", func(t *testing.T) {
		...
		ctx := context.Background()
		em.Tick(ctx)
		testutil.AssertEqual(t, initialElapsed, em.electionElapsed, "Tick should do nothing when stop channel is closed")
	})
	...
	t.Run("TooManyConcurrentOps", func(t *testing.T) {
		...
		initialElapsed := em.electionElapsed
		em.Tick(ctx)
		testutil.AssertEqual(t, initialElapsed, em.electionElapsed, "Tick should be skipped if too many concurrent operations")
	})
}
...
func TestHandleRequestVote(t *testing.T) {
	t.Run("GrantVote_CandidateTermHigher", func(t *testing.T) {
		...
		testutil.AssertEqual(t, types.Term(2), stateMgr.currentTerm, "StateManager current term should update")
		testutil.AssertEqual(t, types.RoleFollower, stateMgr.currentRole, "StateManager role should remain follower")
		testutil.AssertEqual(t, types.NodeID("node2"), stateMgr.leaderID, "StateManager leaderID should update to candidateID")
		testutil.AssertEqual(t, types.NodeID("node2"), stateMgr.votedFor, "StateManager votedFor should update")

		// Check metrics (from stateManager)
		testutil.AssertLen(t, metrics.voteGrantedObservations, 1, "VoteGranted metric should be observed")
		testutil.AssertEqual(t, types.Term(2), metrics.voteGrantedObservations[0], "VoteGranted metric term mismatch")
		testutil.AssertLen(t, metrics.termObservations, 1, "Term observation metric should be observed")
		testutil.AssertEqual(t, types.Term(2), metrics.termObservations[0], "Term observation term mismatch")
	})
	...
	t.Run("RejectVote_LogNotUpToDate", func(t *testing.T) {
		...
		reply, err := em.HandleRequestVote(ctx, args)
		testutil.AssertNoError(t, err, "HandleRequestVote() failed unexpectedly")
		testutil.AssertFalse(t, reply.VoteGranted, "Expected vote to be rejected - log not up to date")
	...
	t.Run("ShuttingDown", func(t *testing.T) {
		...
		reply, err := em.HandleRequestVote(ctx, args)
		testutil.AssertErrorIs(t, err, ErrShuttingDown, "Expected ErrShuttingDown")
		testutil.AssertFalse(t, reply.VoteGranted, "Expected vote to be rejected when shutting down")
		testutil.AssertEqual(t, types.Term(10), reply.Term, "Reply term should be current term even when shutting down")
	})
	...
	t.Run("ContextCancelled", func(t *testing.T) {
		...
		testutil.AssertFalse(t, reply.VoteGranted, "Expected vote to be rejected when context cancelled")
		testutil.AssertEqual(t, types.Term(1), reply.Term, "Reply term should be current term (1)")
	})
}

func TestIsLogUpToDate(t *testing.T) {
	em, _, _, _, _, _, _ := createTestElectionManager(t) // em is not directly used but provides context
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result := em.isLogUpToDate(tc.candidateLastTerm, tc.candidateLastIndex, tc.ourLastTerm, tc.ourLastIndex)
...
func TestResetElectionTimeoutPeriod(t *testing.T) {
	...
	testutil.AssertNotEqual(t, initialPeriod, em.randomizedPeriod, "Expected randomized period to be recalculated and differ")
}
...
func TestRecordVote(t *testing.T) {
	...
	t.Run("RejectedVote", func(t *testing.T) {
		...
		testutil.AssertEqual(t, initialCount, em.voteCount, "Expected vote count to remain unchanged for rejected vote")
		testutil.AssertFalse(t, em.votesReceived["node2"], "Expected rejected vote not to be recorded")
	})
	...
	t.Run("DuplicateVote", func(t *testing.T) {
		...
		testutil.AssertEqual(t, initialCount, em.voteCount, "Expected vote count to remain unchanged for duplicate vote")
	})
}
...
func TestResetVoteTracking(t *testing.T) {
	...
	testutil.AssertLen(t, em.votesReceived, 2, "Pre-condition: votesReceived map should have 2 entries")
...
func TestTrackElectionAttempt(t *testing.T) {
	...
	testutil.AssertEqual(t, initialAttempts+1, em.splitVoteDetector.electionAttempts, "Expected election attempts to increment by 1")
	testutil.AssertLen(t, em.splitVoteDetector.termHistory, 1, "Expected term history length 1")
	testutil.AssertEqual(t, types.Term(5), em.splitVoteDetector.termHistory[0], "Expected term 5 in history")
	...
	testutil.AssertEqual(t, em.splitVoteDetector.maxHistorySize, len(em.splitVoteDetector.termHistory), "Expected history to be trimmed to max size")
	testutil.AssertEqual(t, types.Term(14), em.splitVoteDetector.termHistory[0], "Expected oldest term to be 14 after trimming")
	testutil.AssertEqual(t, 6, em.splitVoteDetector.electionAttempts, "Expected 6 attempts total (1 initial + 5 from loop)")
	...
	testutil.AssertEqual(t, 5, em.splitVoteDetector.electionAttempts, "Expected attempts to be reset to 5 after exceeding 10")
	...
	testutil.AssertEqual(t, 6, em.splitVoteDetector.electionAttempts, "Expected attempts to increment normally from 5")
}
...
func TestResetElectionState(t *testing.T) {
	...
	testutil.AssertEqual(t, int32(ElectionStateIdle), em.electionState.Load(), "Expected election state to be idle")
	...
	testutil.AssertPositive(t, uint64(em.randomizedPeriod), "Expected randomized period to be positive")
}
...
func TestProcessVoteReply(t *testing.T) {
	t.Run("SuccessfulVote", func(t *testing.T) {
		...
		em.processVoteReply("node2", 5, 5, reply) // This will cause em.voteCount to go to 2, achieve quorum, and then be reset to 0 by becomeLeader's cleanup

		testutil.AssertEqual(t, types.RoleLeader, stateMgr.currentRole, "Expected role to become leader after successful quorum")
		testutil.AssertEqual(t, 0, em.voteCount, "Expected vote count to be 0 after successful leader transition and state reset")
		testutil.AssertFalse(t, em.hasQuorum(), "Expected not to have quorum after state reset") // Implicitly checks voteCount is < quorumSize
		testutil.AssertEqual(t, 0, em.splitVoteDetector.consecutiveFails, "Consecutive fails should be reset")
		testutil.AssertEqual(t, 0, em.splitVoteDetector.electionAttempts, "Election attempts should be reset")
	})
	...
	t.Run("HigherTermInReply", func(t *testing.T) {
		...
		t.Logf("DEBUG: Before processVoteReply, initial electionAttempts: %d", em.splitVoteDetector.electionAttempts)
		...
		t.Logf("DEBUG: After processVoteReply, final electionAttempts: %d", em.splitVoteDetector.electionAttempts)

		testutil.AssertEqual(t, types.Term(7), stateMgr.currentTerm, "Expected term to be updated to 7")
		testutil.AssertEqual(t, types.RoleFollower, stateMgr.currentRole, "Expected role to be follower")
		testutil.AssertEqual(t, int32(ElectionStateIdle), em.electionState.Load(), "Election state should be idle after stepping down")
		testutil.AssertEqual(t, initialElectionAttempts+1, em.splitVoteDetector.electionAttempts, "Election attempts should increment due to failure")
	})
	...
	t.Run("NotInVotingState", func(t *testing.T) {
		...
		em.processVoteReply("node2", 5, 5, reply)
		testutil.AssertEqual(t, initialCount, em.voteCount, "Expected vote count to remain unchanged when not in voting state")
	})
	...
	t.Run("VoteRejected", func(t *testing.T) {
		...
		testutil.AssertEqual(t, initialElectionAttempts+1, em.splitVoteDetector.electionAttempts, "Expected election attempts to increment when vote rejected")
		testutil.AssertEqual(t, 0, em.voteCount, "Expected vote count to remain 0 after rejected vote (no self-vote yet)")
	})
}
...
func TestRecordElectionFailure(t *testing.T) {
	...
	testutil.AssertEqual(t, initialFails+1, em.splitVoteDetector.consecutiveFails, "Expected consecutive fails to increment")
	testutil.AssertFalse(t, em.splitVoteDetector.lastFailTime.IsZero(), "Expected last fail time to be set")
}
...
func TestBecomeLeader(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		...
		testutil.AssertEqual(t, 0, em.splitVoteDetector.consecutiveFails, "Consecutive fails should be reset")
		testutil.AssertEqual(t, 0, em.splitVoteDetector.electionAttempts, "Election attempts should be reset")
		testutil.AssertTrue(t, leaderInit.initializeStateCalled, "InitializeLeaderState should be called")
	...
	t.Run("TermMismatch", func(t *testing.T) {
		...
		testutil.AssertNotEqual(t, types.RoleLeader, role, "Should not become leader with term mismatch")
		// Corrected expectation: should become Follower, not remain Candidate
		testutil.AssertEqual(t, types.RoleFollower, role, "Expected role to become follower after term mismatch")
	})
	...
	t.Run("NotCandidate", func(t *testing.T) {
		...
		_, role, _ := stateMgr.GetState()
		testutil.AssertNotEqual(t, types.RoleLeader, role, "Should not become leader when not candidate")
	...
	t.Run("ShuttingDown", func(t *testing.T) {
		...
		_, role, _ := stateMgr.GetState()
		testutil.AssertNotEqual(t, types.RoleLeader, role, "Should not become leader when shutting down")
		// Corrected expectation: should become Follower when shutting down
		testutil.AssertEqual(t, types.RoleFollower, role, "Expected role to become follower after shutting down")
	})
	...
	t.Run("BecomeLeaderFailsInStateManager", func(t *testing.T) {
		...
		// Should not become leader, should step down to follower
		_, role, _ := stateMgr.GetState()
		testutil.AssertNotEqual(t, types.RoleLeader, role, "Should not become leader when BecomeLeader fails")
		testutil.AssertEqual(t, types.RoleFollower, role, "Expected role to become follower after failed transition")
	})
}
...
func TestDefaultStartElection(t *testing.T) {
	...
	t.Run("Success_AchievesQuorumImmediately", func(t *testing.T) {
		...
		testutil.AssertEqual(t, types.RoleLeader, role, "Expected role to be leader immediately after quorum")
	...
	t.Run("ShuttingDown", func(t *testing.T) {
		...
		_, role, _ := stateMgr.GetState()
		testutil.AssertEqual(t, types.RoleFollower, role, "Role should not change when shutting down")
	...
	t.Run("BecomeCandidateFails", func(t *testing.T) {
		...
		_, role, _ := stateMgr.GetState()
		testutil.AssertEqual(t, types.RoleFollower, role, "Should remain follower when BecomeCandidate fails")
		testutil.AssertFalse(t, em.electionInFlight.Load(), "Election should not be in flight")
	})
}
...
func TestDefaultHandleElectionTimeout(t *testing.T) {
	t.Run("Success_StartsElection", func(t *testing.T) {
		...
		testutil.AssertPositive(t, uint64(em.electionCount.Load()), "Expected election count to increment")
		testutil.AssertNotEqual(t, int64(0), em.lastElectionTime.Load(), "Expected last election time to be updated")
	})
	...
	t.Run("ShuttingDown", func(t *testing.T) {
		...
		testutil.AssertEqual(t, uint64(0), em.electionCount.Load(), "Should not start election when shutting down")
	})
	...
	t.Run("ElectionInFlight", func(t *testing.T) {
		...
		testutil.AssertEqual(t, uint64(0), em.electionCount.Load(), "Should not start another election if one is already in flight")
	})
	...
	t.Run("TooSoonAfterLastElection", func(t *testing.T) {
		...
		testutil.AssertEqual(t, uint64(0), em.electionCount.Load(), "Should not start election if too soon after last one")
	...
	t.Run("NotFollower", func(t *testing.T) {
		...
		testutil.AssertEqual(t, uint64(0), em.electionCount.Load(), "Should not start election when not a follower")
	})
	...
	t.Run("NotIdleState", func(t *testing.T) {
		...
		testutil.AssertEqual(t, uint64(0), em.electionCount.Load(), "Should not start election if not in idle state")
	})
}
...
func TestBroadcastVoteRequests(t *testing.T) {
	t.Run("Success_SendsToAllPeers", func(t *testing.T) {
		...
		testutil.AssertEqual(t, len(expectedPeers), len(callCounts), "Expected calls only to other peers")
	})
...
func TestSendVoteRequest(t *testing.T) {
	t.Run("Success_VoteGranted", func(t *testing.T) {
		...
		testutil.AssertEqual(t, 1, em.voteCount, "Expected vote count 1 after successful request and processing")
		testutil.AssertEqual(t, 0, em.splitVoteDetector.electionAttempts, "Election attempts should not increment on success")
		testutil.AssertTrue(t, networkMgr.callCount >= 1, "Network manager SendRequestVote should have been called")
	})
	...
	t.Run("NetworkError", func(t *testing.T) {
		...
		testutil.AssertEqual(t, initialElectionAttempts+1, em.splitVoteDetector.electionAttempts, "Expected election attempts to increment on network error")
		testutil.AssertTrue(t, networkMgr.callCount >= 1, "Network manager SendRequestVote should have been called")
	})
	...
	t.Run("ReplyHigherTerm_StepsDown", func(t *testing.T) {
		...
		networkMgr.sendRequestVoteFunc = func(ctx context.Context, target types.NodeID, args *types.RequestVoteArgs) (*types.RequestVoteReply, error) {
			return &types.RequestVoteReply{Term: 7, VoteGranted: false}, nil // Peer returns higher term
		}
		...
		testutil.AssertEqual(t, types.Term(7), stateMgr.currentTerm, "StateManager term should update to higher term from reply")
		testutil.AssertEqual(t, types.RoleFollower, stateMgr.currentRole, "StateManager role should be follower after stepping down")
		testutil.AssertEqual(t, int32(ElectionStateIdle), em.electionState.Load(), "Election state should reset to idle")
	})
}
...
func TestApplyDefaults(t *testing.T) {
	t.Run("InvalidElectionTickCount", func(t *testing.T) {
		...
		testutil.AssertEqual(t, DefaultElectionTickCount, em.electionTickCount, "Expected default election tick count")
	})
	...
	t.Run("LowRandomizationFactor", func(t *testing.T) {
		...
		testutil.AssertEqual(t, DefaultElectionRandomizationFactor, em.randomizationFactor, "Expected default randomization factor")
	})
	...
	t.Run("ElectionTickCountLessThanHeartbeatTickCount", func(t *testing.T) {
		...
		testutil.AssertEqual(t, 3, em.electionTickCount, "Expected electionTickCount to remain 3 if > 0 but < HeartbeatTickCount")
	})
	...
	t.Run("ValidConfiguration", func(t *testing.T) {
		...
		testutil.AssertEqual(t, originalTickCount, em.electionTickCount, "Valid tick count should not change")
		testutil.AssertEqual(t, originalRandomization, em.randomizationFactor, "Valid randomization factor should not change")
	})
}
...
func TestStop(t *testing.T) {
	...
	t.Run("WithConcurrentOperations_CompletesGracefully", func(t *testing.T) {
		...
		testutil.AssertTrue(t, elapsed >= 50*time.Millisecond, "Stop should wait for concurrent operations to complete")
		testutil.AssertTrue(t, elapsed < 2*time.Second, "Stop waited too long, expected less than 2 seconds")
	})
	...
	t.Run("WithConcurrentOperations_Timeout", func(t *testing.T) {
		...
		testutil.AssertTrue(t, elapsed >= 2*time.Second, "Stop should timeout after the configured period")
		testutil.AssertTrue(t, elapsed < 3*time.Second, "Stop timeout too long, expected around 2 seconds")
		testutil.AssertEqual(t, int32(1), em.concurrentOps.Load(), "Concurrent operations count should remain 1 after timeout")
	})
}
...
func TestElectionManagerEdgeCases(t *testing.T) {
	t.Run("ZeroQuorumSize", func(t *testing.T) {
		...
		testutil.AssertContains(t, err.Error(), "quorum size must be positive", "Error message should be descriptive")
	})
	...
	t.Run("EmptyPeersMap", func(t *testing.T) {
		...
		testutil.AssertLen(t, em.(*electionManager).peers, 0, "Peers map should be empty") // Only self if it was in the map, but it's not here initially.
	})
	...
	t.Run("QuorumLargerThanAvailablePeers", func(t *testing.T) {
		...
		testutil.AssertFalse(t, em.hasQuorum(), "Should not have quorum when quorum size > available votes")
	})
}
